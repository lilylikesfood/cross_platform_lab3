...
const ContactListScreen = ({ navigation = { navigate: () => {} } }) => {
  ...
  const displayContacts = useMemo(() => {
    ...
    // Favorites first
    return filtered.sort((a, b) => (a.favorite && !b.favorite ? -1 : !a.favorite && b.favorite ? 1 : 0));
  ...
  const renderEmptyState = useCallback(() => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>
        {searchTerm ? `No contacts match "${searchTerm}"` : 'Add your first contact to get started'}
      </Text>
    </View>
  ), [searchTerm]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.statsContainer}>
        <Text style={styles.statsText}>
          {displayContacts.length} contact{displayContacts.length !== 1 ? 's' : ''}{searchTerm && ` for "${searchTerm}"`}
        </Text>
      </View>
      ...
      <FlatList
        ...
        contentContainerStyle={[styles.listContainer, displayContacts.length === 0 && styles.emptyListContainer]}
      ...
      {/* Floating Add Button */}
<TouchableOpacity
  style={[styles.floatingButton, { zIndex: 9999, elevation: 10 }]}
  onPress={() => {
    console.log('Floating + pressed');
    // Guard - if navigation is available, navigate. Otherwise show a helpful alert.
    if (navigation && typeof navigation.navigate === 'function') {
      navigation.navigate('AddContact');
    } else {
      Alert.alert('Navigation unavailable', 'AddContact screen not registered or navigation prop missing.');
    }
  }}
...
const styles = StyleSheet.create({
  ...
  statsText: { fontSize: Fonts.small, color: Colors.text.secondary },
  listContainer: { paddingBottom: 80 },
  emptyListContainer: { flexGrow: 1, justifyContent: 'center' },
  emptyContainer: { ...GlobalStyles.centered, paddingHorizontal: Spacing.xl },
  emptyTitle: { fontSize: Fonts.large, fontWeight: 'bold', color: Colors.text.primary, marginTop: Spacing.md, marginBottom: Spacing.sm },
  emptyText: { fontSize: Fonts.medium, color: Colors.text.secondary, textAlign: 'center', lineHeight: 22 },
  floatingButton: {
    position: 'absolute',
    bottom: 20,
    right: 20,
...
//       {/* ScrollView Contacts */}
//       <ScrollView
//         contentContainerStyle={[styles.scrollContainer, displayContacts.length === 0 && styles.emptyListContainer]}
//         refreshControl={
//           <RefreshControl
//             refreshing={refreshing}
//             onRefresh={handleRefresh}
...
//         showsVerticalScrollIndicator={false}
//       >
//         {displayContacts.length === 0 ? renderEmptyState() : 
